...
const AVAILABLE_MODELS = [
  ...
  },
  {
    ...
    contextWindow: "128K tokens",
    badge: "Meta AI Flagship",
    category: "Open Source Flagship",
    provider: "meta",
    supportsSearch: true,
    supportsVision: true,
  },
  {
    id: "llama-3.2-vision",
    name: "Meta Llama 3.2 Vision (Meta AI)",
    description: "Meta's official multimodal vision model for diagram inspection, UI breakdown, and document parsing.",
    contextWindow: "128K tokens",
    badge: "Meta Vision",
  ...
  {
    id: "gemini-2.5-flash",
    name: "Shawez Turbo 2.5 (3-Key Pool)",
    description: "Next-gen hybrid model with breakthrough speed, zero quota limits, 3-key auto-failover, and native image/document OCR.",
    contextWindow: "1M tokens",
    badge: "Ultra Fast",
  ...
  {
    id: "gemini-2.5-pro",
    name: "Shawez Deep Pro 2.5",
    description: "Maximum reasoning power for complex mathematics, architectural design, and deep technical logic with 2M context.",
...
app.post("/api/chat/stream", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (m.includes("claude") || m.includes("sonnet")) {
      personaInstruction = "You are Claude 3.5 Sonnet, an elite AI model known for world-class coding, system architecture, deep reasoning, and precise problem solving. Provide elegant, structured, and accurate responses.";
    } else if (m.includes("gpt-4") || m.includes("chatgpt")) {
      personaInstruction = "You are ChatGPT GPT-4o, a highly capable and intelligent AI flagship known for versatile reasoning, full-stack programming, and clear, human-like explanations.";
    } else if (m.includes("deepseek") || m.includes("r1")) {
      personaInstruction = "You are DeepSeek-R1, an advanced reasoning AI specializing in transparent step-by-step logic, mathematical proofs, and optimized algorithms.";
    } else if (m.includes("llama")) {
      personaInstruction = "You are Meta Llama 3.3 70B, a powerful flagship open model known for high performance in coding, reasoning, and multilingual conversation.";
    } else {
