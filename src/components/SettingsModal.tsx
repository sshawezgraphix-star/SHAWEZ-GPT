...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  // In-App Live Auto-Updater State
  ...
  const handleCheckUpdate = async () => {
    ...
    try {
      ...
      setUpdateInfo(info);
      if (info.hasUpdate) {
        setUpdateFeedback(`🎉 New Version Available: v${info.latestVersion}!`);
      } else {
        setUpdateFeedback(`✅ You are using the latest version (v${CURRENT_APP_VERSION})!`);
      }
    } catch {
      setUpdateFeedback("Failed to check for updates. Please verify your internet connection.");
    } finally {
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1 text-sm text-slate-700 dark:text-slate-300">
          ...
          {activeTab === "system" && (
            <div className="space-y-5">
              ...
              {/* SECTION 0A: In-App Auto Updater (1-Click Update) */}
              <div className="p-4 rounded-2xl bg-gradient-to-br from-emerald-500/10 via-teal-500/5 to-transparent border border-emerald-500/30 space-y-3 shadow-xs">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Download className="w-4 h-4 text-emerald-500" />
                    <span className="font-bold text-xs text-slate-900 dark:text-white uppercase tracking-wider">
                      In-App Auto-Updater (1-Click Update)
                    </span>
                  </div>
                  <span
                  ...
                  >
                    {updateInfo?.hasUpdate ? `Update Available: v${updateInfo.latestVersion}` : `v${CURRENT_APP_VERSION} (Latest)`}
                  </span>
                </div>

                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 p-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">Installed Version:</span>
                      <span className="font-mono text-xs font-bold text-emerald-600 dark:text-emerald-400">v{CURRENT_APP_VERSION}</span>
                    </div>
                    {updateInfo && (
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">Cloud Latest:</span>
                        <span className="font-mono text-xs font-bold text-slate-900 dark:text-white">v{updateInfo.latestVersion}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto">
                    <button
                      ...
                      className="flex-1 sm:flex-none px-3.5 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-xs font-semibold text-slate-700 dark:text-slate-200 transition-colors flex items-center justify-center gap-1.5"
                    >
                      <RefreshCw className={`w-3.5 h-3.5 ${isCheckingUpdate ? "animate-spin text-emerald-500" : ""}`} />
                      <span>Check Now</span>
                    </button>

                    <button
                      onClick={() => triggerAppUpdate(updateInfo?.downloadUrl)}
                      className="flex-1 sm:flex-none px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 active:scale-95 text-white text-xs font-bold whitespace-nowrap shadow-md shadow-emerald-600/20 transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                    >
                      <Download className="w-3.5 h-3.5" />
                      <span>1-Click Update App</span>
                    </button>
                  </div>
                </div>
                ...
                {updateFeedback && (
                  <div
                    className={`p-2.5 rounded-xl text-xs font-medium ${
                      updateFeedback.includes("Available")
                        ? "bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20"
                        : updateFeedback.includes("latest")
              ...
              {/* SECTION 0B: Mobile App Live Cloud Sync */}
